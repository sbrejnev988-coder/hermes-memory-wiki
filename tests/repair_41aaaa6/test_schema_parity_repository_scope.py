...
def test_native_and_wrapper_patch_outcome_require_repository_id(tmp_path, monkeypatch):
    ...
    native = next(
        ...
        if item.get('name') == 'memory_wiki_patch_outcome_add'
    )
    assert 'repository_id' in native['parameters']['required']

    wrapper = json.loads(
        (root / 'mcp-wrapper' / 'tool_schemas.json').read_text(encoding='utf-8')
    )
    wrapped = next(item for item in wrapper if item.get('name') == 'mw_patch_outcome_add')
    assert 'repository_id' in wrapped['inputSchema']['required']
