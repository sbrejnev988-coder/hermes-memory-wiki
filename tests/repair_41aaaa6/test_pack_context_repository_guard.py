...
def test_pack_context_excludes_foreign_repository_and_keeps_global_preference(tmp_path, monkeypatch):
    ...
    preference_id = provider._add_claim(
        "The user prefers concise verification summaries", "preferences",
        "Explicit durable user preference", "explicit_user", 0.95, 0.9,
    )
    raw = provider.handle_tool_call("memory_wiki_pack_context", {
        "query": "foreign implementation concise verification summaries",
        "max_tokens": 2000, "output_mode": "debug",
        "coverage_manifest": {"protocol_version": 2, "repository_id": "repo-A", "covered": []},
    })
...
def test_pack_context_exact_source_suppresses_duplicate_even_with_contract_after_it(tmp_path, monkeypatch):
    ...
    raw = provider.handle_tool_call("memory_wiki_pack_context", {
        "query": "Service uses port 1234", "max_tokens": 2000, "output_mode": "debug",
        "coverage_manifest": {
            "protocol_version": 2, "repository_id": "repo-A",
            "covered": [
                {
                    "kind": "exact_source", "file_path": "src/shared.py", "symbol_id": "sym_shared",
                    "revision": "rev1", "content_hash": digest,
                    "hash_algorithm": "sha256", "canonicalization": "utf8-raw", "content_kind": "source",
                },
                {
                    "kind": "contract", "file_path": "src/shared.py", "symbol_id": "sym_shared",
    ...
    suppressed = result.get("suppression_manifest", {}).get("suppressed", [])
    assert any(item.get("claim_id") == claim["id"] and item.get("reason") == "exact_content_hash_match" for item in suppressed)
