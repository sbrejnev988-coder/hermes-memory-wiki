...
# Проверяем, что runtime жёстко ограничивает ошибочно завышенные env-настройки.
...
os.environ["MEMORY_WIKI_EMBED_DIMENSIONS"] = "3"
os.environ["MEMORY_WIKI_VECTOR_SIZE"] = "3"

PLUGIN = Path(__file__).resolve().parents[1] / "__init__.py"
spec = importlib.util.spec_from_file_location(
    "memory_wiki_prefetch_test",
    str(PLUGIN),
...
def test_health_stale_while_revalidate() -> None:
    ...
    mod._OPENROUTER_HEALTH_CACHE.update(
        {"checked_at": time.time() - 999, "available": False, "refreshing": False}
    )
