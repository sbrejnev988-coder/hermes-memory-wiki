...
def main() -> int:
    ...
    try:
        ...
        required = [
            ...
            "memory_wiki_rebuild_from_journal",
        ]
        ...
        journal_status = call("memory_wiki_journal_status", {"verify": True, "limit": 8})
        assert journal_status.get("events_valid", 0) > 0 and journal_status.get("hash_errors") == 0, journal_status
        assert Path(journal_status.get("journal_path", "")).exists(), journal_status
        checkpoint = call("memory_wiki_journal_checkpoint", {"name": "smoke"})
        assert Path(checkpoint.get("path", "")).exists() and checkpoint.get("journal_seq", 0) >= journal_status.get("meta", {}).get("seq", 0), checkpoint
        rebuild_plan = call("memory_wiki_rebuild_from_journal", {"apply": False})
        assert "events_to_replay" in rebuild_plan and rebuild_plan.get("checkpoint"), rebuild_plan
